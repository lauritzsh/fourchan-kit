...
module FourchanKit
  ...
  class CLI < Thor
    ...
    def download
      ...
      if url
        FourchanKit::Tools.download_thread(url, options.dup)
      elsif file
        FourchanKit::Tools.download_threads(file, options.dup)
      else
    ...
    def lurk(thread)
      options[:refresh] >= 5 ? Fourgem::Tools.lurk(thread, options.dup) : puts("Be fair, have refresh >= 5")
    end
  end
end
